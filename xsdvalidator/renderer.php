...
/**
 * xsdvalidator question renderer class.
 *
...
/**
 * Generates the output for xsdvalidator questions.
 *
 * @copyright 2013 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_xsdvalidator_renderer extends qtype_with_combined_feedback_renderer {
    public function formulation_and_controls(question_attempt $qa,
        ...
        $attributes = array(
            'id'    => $inputname,
            'name'  => $inputname,
            'class' => 'qtype_xsdvalidator_response',
            'rows'  => 5,
            'cols'  => 60,
        );
        if ($options->readonly) {
        ...
        $table->head = array(get_string('sentence', 'qtype_xsdvalidator'), get_string('shouldmatch', 'qtype_xsdvalidator'));
        if ($options->correctness || $options->feedback) {
            $table->head[] = get_string('doesmatch', 'qtype_xsdvalidator');
        }
        foreach ($question->sentences as $sentence => $shouldmatch) {
            ...
            if ($options->correctness || $options->feedback) {
                $doesmatch = $question->xml_validates_xsd($sentence, $expression);
