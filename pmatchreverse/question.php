...
class qtype_pmatchreverse_question extends question_graded_automatically_with_countback {
    ...
    public function classify_response(array $response) {
      ...
      $expression =$response['answer'];
        ...
        $parts = array();
        foreach ($this->sentences as $sentence => $shouldmatch) {
            $doesmatch = $this->sentence_matches_express($sentence, $expression);
    ...
    public function sentence_matches_expression($sentence, $expression) {
      /*  if (!$expression->is_valid()) {
        ...
        return true;//$expression->matches(new pmatch_parsed_string($sentence));

    }


    public function sentence_matches_express($sentence, $expression) {
   ...
   return false;
    ...
    public function grade_response(array $response) {
      //  $expression = $this->parse_expression($response['answer']);
        ...
        $numright = 0;
        foreach ($this->sentences as $sentence => $shouldmatch) {
            if ($this->compare_bools($shouldmatch, $this->sentence_matches_express($sentence, $expression))) {
                $numright += 1;
            }
        }
